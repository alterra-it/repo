...
Функция ТекстЗапросаОЗаказахПоставщику(СоответствиеЗапросыДанные,НомерПоследнегоПакета)
	...
	|				ИЛИ НЕ &ИспользоватьСтатусыЗаказовПоставщикам)
	...
	|				И ДокументЗаказПоставщику.СуммаДокумента > 0
	|				И (ЕСТЬNULL(РасчетыСПоставщикамиОстатки.КОплатеПриход, 0) >= ДокументЗаказПоставщику.СуммаПредоплатыДоПоступления + ДокументЗаказПоставщику.СуммаАвансаДоПодтверждения
	|					ИЛИ ДокументЗаказПоставщику.СуммаПредоплатыДоПоступления = 0
	|					ИЛИ ДокументЗаказПоставщику.ПорядокРасчетов <> ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным))
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовПоставщикам.ГотовКПоступлению)
...
Функция ТекстЗапросаОЗаказахПартнера(СоответствиеЗапросыДанные,НомерПоследнегоПакета)
	...
	|				ИЛИ НЕ &ИспользоватьСтатусыЗаказовКлиентов)
	...
	|						ДокументЗаказКлиента.Проведен
	|						И ДокументЗаказКлиента.Партнер = &Партнер
	|						И ДокументЗаказКлиента.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.Закрыт))) КАК ЗаказыКлиентовОстатки
	...
	|						ДокументЗаказКлиента.Проведен
	|						И ДокументЗаказКлиента.Партнер = &Партнер
	|						И ДокументЗаказКлиента.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.Закрыт))) КАК ЗаказыКлиентовОстаткиНаДатуАктуальности
...
Функция ТекстЗапросаОДоступныхСоглашениях(СоответствиеЗапросыДанные,НомерПоследнегоПакета)
	...
	Возврат "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	        ...
	        |УПОРЯДОЧИТЬ ПО
	        |	Типовое,
	        |	СправочникСоглашенияСКлиентами.СегментПартнеров УБЫВ,
	        |	СправочникСоглашенияСКлиентами.Организация
	        |ИТОГИ
	        |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Ссылка)
	        |ПО
	        |	ОБЩИЕ
...
Функция ТекстЗапросаОДоговорахКонтрагентов(СоответствиеЗапросыДанные,НомерПоследнегоПакета)
	...
	НомерПоследнегоПакета = НомерПоследнегоПакета + 1;
	// Устинов В.А. 20.06.2014 15:28:53
	// сортировка
	Возврат "ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
//		Запрос.Текст = Запрос.Текст + ТекстЗапросаОВзаиморасчетахСПоставщиком(СоответствиеЗапросыДанные,НомерПоследнегоПакета);
	...
	ИспользоватьСтатусыЗаказовКлиентов    = ПолучитьФункциональнуюОпцию("УдалитьИспользоватьСтатусыЗаказовКлиентов");
	ИспользоватьСтатусыЗаказовПоставщикам = ПолучитьФункциональнуюОпцию("ИспользоватьСтатусыЗаказовПоставщикам");
	Запрос.УстановитьПараметр("ИспользоватьСтатусыЗаказовКлиентов",    ИспользоватьСтатусыЗаказовКлиентов);
	Запрос.УстановитьПараметр("ИспользоватьСтатусыЗаказовПоставщикам", ИспользоватьСтатусыЗаказовПоставщикам);
...
Процедура ВывестиДанныеОВзаиморасчетахСКлиентом(РезультатЗапросаПоВзаиморасчетам,ТаблицаОтчета, Макет)
	...
	Если Не ПустаяСтрока(СтрокаНашДолг) Тогда
		ТекстДолгКлиента = ТекстДолгКлиента + ?(ПустаяСтрока(ТекстДолгКлиента), "", Символы.ПС) + НСтр("ru='Общая наша задолженность перед клиентом '") + Лев(СтрокаНашДолг,СтрДлина(СтрокаНашДолг)-2) + ".";
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(СтрокаПросроченнаяЗадолженность) Тогда
		...
		СтрокаПросроченнаяЗадолженность = НСтр("ru='Просроченная задолженность как клиента'") + " " + Лев(СтрокаПросроченнаяЗадолженность,СтрДлина(СтрокаПросроченнаяЗадолженность)-2) + ".";
