...
Процедура ЗаполнитьДокументыНаЭкспорт(
	...
	НачПериода,
	КонПериода,
	ПараметрыЭкспорта) Экспорт
	
	ТекстЗапроса = ПолучитьТекстЗапросаПоСписаниеБезналичныхДенежныхСредств();
	...
	ЗапросПоДокументам.УстановитьПараметр("Организация",               ПараметрыЭкспорта.Организация);
	ЗапросПоДокументам.УстановитьПараметр("БИКБанка",                  ПараметрыЭкспорта.БИКБанка);
	ЗапросПоДокументам.УстановитьПараметр("Программа",                 ПараметрыЭкспорта.Программа);
	ЗапросПоДокументам.УстановитьПараметр("ПрямойОбмен",               ПараметрыЭкспорта.ПрямойОбмен);
	ЗапросПоДокументам.УстановитьПараметр("ПлатежноеПоручение",        Перечисления.ТипыПлатежныхДокументов.ПлатежноеПоручение);
	ЗапросПоДокументам.УстановитьПараметр("Ссылка",                    Неопределено);
	ЗапросПоДокументам.УстановитьПараметр("ИспользоватьОтборПоСсылке", Ложь);
...
Процедура ЗаполнитьДокументыНаИмпорт(
				...
				Для Каждого СтрокаВыписки Из ВыпискаБанка.Строки Цикл
					Если СтрокаВыписки.Реквизит = "СтрокаТЧ" И СтрокаВыписки.Строки.Количество() > 0 Тогда
						...
						Для Каждого СтрокаРеквизита Из СтрокаВыписки.Строки Цикл
							...
							Если НаборКолонокДокументыКИмпорту.Найти(НазваниеРеквизита) <> Неопределено Тогда
								НоваяСтрокаДокументов[НазваниеРеквизита] = СтрокаРеквизита.ЗначениеРеквизита;
							КонецЕсли;
						КонецЦикла;
	...
	Для Каждого СтрокаДокумента Из ВозвращаемыеДанные.ДокументыКИмпорту Цикл
		...
		Если НЕ СчетПринадлежитОрганизации(СтрокаДокумента.ПлательщикСчет, Организация, БанковскийСчет)
			И НЕ СчетПринадлежитОрганизации(СтрокаДокумента.ПолучательСчет, Организация)Тогда
	...
	Для Каждого СтрокаДокумента Из СтрокиКУдалению Цикл
		ВозвращаемыеДанные.ДокументыКИмпорту.Удалить(СтрокаДокумента);
	КонецЦикла;
...
Процедура Загрузить(ДокументыКИмпорту, 
	...
	Для Каждого СтрокаСекции Из ДокументыКИмпорту Цикл
		...
		Если ЗначениеЗаполнено(СтрокаСекции.Документ) И
			(ТипЗнч(СтрокаСекции.Документ) <> Тип("ДокументСсылка.ПоступлениеБезналичныхДенежныхСредств")
			Или ТипЗнч(СтрокаСекции.Документ) <> Тип("ДокументСсылка.СписаниеБезналичныхДенежныхСредств")) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если СтрокаСекции.Загружать Тогда
			...
			Если ПустаяСтрока(РезультатПроверки) Тогда
				...
				Если СтрокаСекции.ВидОперации = Перечисления.ХозяйственныеОперации.СдачаДенежныхСредствВБанк
				 ИЛИ СтрокаСекции.ВидОперации = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзБанка Тогда
					Продолжить; // не требуется созданение документа
					
				ИначеЕсли ЗначениеЗаполнено(СтрокаСекции.СуммаСписано) Тогда
...
Процедура ЗаполнитьДанныеПоПлатежномуПоручениюДляЭД(СсылкаНаДокумент, СтруктураПараметров) Экспорт
	
	ЗапросДанныхДокумента = Новый Запрос(
		ПолучитьТекстЗапросаПоСписаниеБезналичныхДенежныхСредств());
	ЗапросДанныхДокумента.УстановитьПараметр("ДатаНачало",                '00010101');
	ЗапросДанныхДокумента.УстановитьПараметр("ДатаОкончание",             '00010101');
	ЗапросДанныхДокумента.УстановитьПараметр("Организация",               Неопределено);
	ЗапросДанныхДокумента.УстановитьПараметр("Банк",                      Неопределено);
	ЗапросДанныхДокумента.УстановитьПараметр("БИКБанка",                  Неопределено);
	ЗапросДанныхДокумента.УстановитьПараметр("Программа",                 Неопределено);
	ЗапросДанныхДокумента.УстановитьПараметр("ПрямойОбмен",               Ложь);
	ЗапросДанныхДокумента.УстановитьПараметр("ПлатежноеПоручение",        Неопределено);
	ЗапросДанныхДокумента.УстановитьПараметр("Ссылка",                    СсылкаНаДокумент);
	ЗапросДанныхДокумента.УстановитьПараметр("ИспользоватьОтборПоСсылке", Истина);
	
	Выборка = ЗапросДанныхДокумента.Выполнить().Выбрать();
	
	Если Выборка.Следующий() Тогда
		ЗаполнитьЗначениеЭкспортаДляЭД(СтруктураПараметров, Выборка);
	КонецЕсли;
...
Процедура ЗаполнитьЗначениеЭкспортаДляЭД(СтрокаЭкспорта, ВыборкаДляЭкспорта)
	
	Плательщик = "Организация";
	Получатель = "Контрагент";
	
	СтрокаЭкспорта.ИдДокумента = "";
	СтрокаЭкспорта.Номер = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(
		ВыборкаДляЭкспорта.Номер,
		Истина, // УдалитьПрефиксИнформационнойБазы
		Истина); // УдалитьПользовательскийПрефикс
	...
	СтрокаЭкспорта.НепрямыеРасчетыУПлательщика = ЗначениеЗаполнено(ВыборкаДляЭкспорта[Плательщик + "БанкДляРасчетов"]);
	СтрокаЭкспорта.НепрямыеРасчетыУПолучателя  = ЗначениеЗаполнено(ВыборкаДляЭкспорта[Получатель + "БанкДляРасчетов"]);
	...
	СтрокаЭкспорта.ПлательщикНаим = ВыборкаДляЭкспорта[Плательщик + "Наим"];
	СтрокаЭкспорта.ПлательщикИНН = ВыборкаДляЭкспорта[Плательщик + "ИНН"];
	СтрокаЭкспорта.ПлательщикРасчСчет = ВыборкаДляЭкспорта[Плательщик + "РасчСчет"];
	СтрокаЭкспорта.ПлательщикБанк = ВыборкаДляЭкспорта[Плательщик + "Банк"];
	СтрокаЭкспорта.ПлательщикГородБанка = ВыборкаДляЭкспорта[Плательщик + "ГородБанка"];
	СтрокаЭкспорта.Плательщик = "ИНН " + ВыборкаДляЭкспорта[Плательщик + "ИНН"] + " " + СтрокаЭкспорта.ПлательщикНаим;
	Если СтрокаЭкспорта.НепрямыеРасчетыУПлательщика Тогда
		СтрокаЭкспорта.Плательщик = СтрокаЭкспорта.Плательщик
			+ " р/с " + СтрокаЭкспорта.ПлательщикРасчСчет + " в "
			+ СтрокаЭкспорта.ПлательщикБанк + " " + СтрокаЭкспорта.ПлательщикГородБанка;
	КонецЕсли;
	СтрокаЭкспорта.ПлательщикКоррСчет = ВыборкаДляЭкспорта[Плательщик + "КоррСчет"];
	СтрокаЭкспорта.ПлательщикБанкДляРасчетов = ВыборкаДляЭкспорта[Плательщик + "БанкДляРасчетов"];
	СтрокаЭкспорта.ПлательщикГородРЦБанка = ВыборкаДляЭкспорта[Плательщик + "ГородРЦБанка"];
	СтрокаЭкспорта.ПлательщикБИКРЦБанка = ВыборкаДляЭкспорта[Плательщик + "БИКРЦБанка"];
	СтрокаЭкспорта.ПлательщикКоррСчетРЦБанка = ВыборкаДляЭкспорта[Плательщик + "КоррСчетРЦБанка"];
	СтрокаЭкспорта.ПлательщикБИКБанка = ВыборкаДляЭкспорта[Плательщик + "БИКБанка"];
	
	СтрокаЭкспорта.ПолучательНаим = ВыборкаДляЭкспорта[Получатель];
	СтрокаЭкспорта.ПолучательИНН = ВыборкаДляЭкспорта[Получатель + "ИНН"];
	СтрокаЭкспорта.ПолучательРасчСчет = ВыборкаДляЭкспорта[Получатель + "РасчСчет"];
	СтрокаЭкспорта.ПолучательБанк = ВыборкаДляЭкспорта[Получатель + "Банк"];
	СтрокаЭкспорта.ПолучательГородБанка = ВыборкаДляЭкспорта[Получатель + "ГородБанка"];
	СтрокаЭкспорта.Получатель = "ИНН " + ВыборкаДляЭкспорта[Получатель + "ИНН"] + " " + СтрокаЭкспорта.ПолучательНаим;
	Если СтрокаЭкспорта.НепрямыеРасчетыУПолучателя Тогда
		СтрокаЭкспорта.Получатель = СтрокаЭкспорта.Получатель + " р/с " + СтрокаЭкспорта.ПолучательРасчСчет + " в "
			+ СтрокаЭкспорта.ПолучательБанк + " " + СтрокаЭкспорта.ПолучательГородБанка;
	КонецЕсли;
	СтрокаЭкспорта.ПолучательКоррСчет = ВыборкаДляЭкспорта[Получатель + "КоррСчет"];
	СтрокаЭкспорта.ПолучательБанкДляРасчетов = ВыборкаДляЭкспорта[Получатель + "БанкДляРасчетов"];
	СтрокаЭкспорта.ПолучательГородРЦБанка = ВыборкаДляЭкспорта[Получатель + "ГородРЦБанка"];
	СтрокаЭкспорта.ПолучательБИКРЦБанка = ВыборкаДляЭкспорта[Получатель + "БИКРЦБанка"];
	СтрокаЭкспорта.ПолучательКоррСчетРЦБанка = ВыборкаДляЭкспорта[Получатель + "КоррСчетРЦБанка"];
	СтрокаЭкспорта.ПолучательБИКБанка = ВыборкаДляЭкспорта[Получатель + "БИКБанка"];
	
	СтрокаЭкспорта.ПеречислениеВБюджет = ВыборкаДляЭкспорта.ПеречислениеВБюджет;
	...
	Если (ВыборкаДляЭкспорта.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеВБюджет
		...
		СтрокаЭкспорта.КодБК = ВыборкаДляЭкспорта.КодБК;
		СтрокаЭкспорта.КодОКАТО = ВыборкаДляЭкспорта.КодОКАТО;
...
Функция ПолучитьТекстЗапросаПоСписаниеБезналичныхДенежныхСредств()
	...
	|	И ВЫБОР КОГДА БанковскиеСчетаОрганизаций.РучноеИзменениеРеквизитовБанка = ИСТИНА 
	|		ТОГДА БанковскиеСчетаОрганизаций.БИКБанка = &БИКБанка
	|		ИНАЧЕ БанковскиеСчетаОрганизаций.Банк.Код = &БИКБанка КОНЕЦ
	|	И БанковскиеСчетаОрганизаций.ИспользоватьПрямойОбменСБанком = &ПрямойОбмен
	|	И ВЫБОР КОГДА &ПрямойОбмен ТОГДА
	|		БанковскиеСчетаОрганизаций.ИспользоватьОбменСБанком = ЛОЖЬ
	|	ИНАЧЕ
	|		БанковскиеСчетаОрганизаций.ИспользоватьОбменСБанком = ИСТИНА
	|	КОНЕЦ
	|	И (БанковскиеСчетаОрганизаций.Программа = &Программа ИЛИ &ПрямойОбмен)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|
	...
	|	ВЫБОР
	|		КОГДА (ВЫРАЗИТЬ(СписаниеБезналичныхДенежныхСредств.НомерВходящегоДокумента КАК СТРОКА(11))) = """"
	...
	|			ТОГДА ВЫБОР
	|					КОГДА СписаниеБезналичныхДенежныхСредств.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику))
	|							ИЛИ СписаниеБезналичныхДенежныхСредств.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыплатаЗарплатыНаЛицевыеСчета))
	...
	|				ИЛИ СписаниеБезналичныхДенежныхСредств.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыплатаЗарплатыНаЛицевыеСчета))
	|			ТОГДА СписаниеБезналичныхДенежныхСредств.ПодотчетноеЛицо.Наименование
	|		ИНАЧЕ СписаниеБезналичныхДенежныхСредств.Контрагент.Наименование
	|	КОНЕЦ КАК КонтрагентКратко,
	|	ВЫБОР
	|		КОГДА СписаниеБезналичныхДенежныхСредств.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику))
	|				ИЛИ СписаниеБезналичныхДенежныхСредств.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыплатаЗарплатыНаЛицевыеСчета))
	|			ТОГДА СписаниеБезналичныхДенежныхСредств.ПодотчетноеЛицо.ИНН
	|		ИНАЧЕ СписаниеБезналичныхДенежныхСредств.Контрагент.ИНН
	...
	|	ИЛИ (НЕ &ИспользоватьОтборПоСсылке
	|		И СписаниеБезналичныхДенежныхСредств.Дата МЕЖДУ &ДатаНачало И &ДатаОкончание
	|		И НЕ СписаниеБезналичныхДенежныхСредств.ПометкаУдаления
	|		И СписаниеБезналичныхДенежныхСредств.ТипПлатежногоДокумента = &ПлатежноеПоручение)
	|		И СписаниеБезналичныхДенежныхСредств.БанковскийСчет В (ВЫБРАТЬ Ссылка ИЗ БанковскиеСчета))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	&СтрокаСписаниеБезналичных,
	...
	|	СписаниеБезналичныхДенежныхСредств.ПоказательНомера,
	|	СписаниеБезналичныхДенежныхСредств.ПоказательДаты,
	...
	|	КОНЕЦ,
	|	СписаниеБезналичныхДенежныхСредств.Организация.Наименование,
	...
	|		ИНАЧЕ СписаниеБезналичныхДенежныхСредств.БанковскийСчетПолучатель.ТекстКорреспондента
	|	КОНЕЦ,
	|	СписаниеБезналичныхДенежныхСредств.БанковскийСчетПолучатель.Владелец.Наименование,
	...
	Если ВсеРеквизитыПоискаЕсть Тогда
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			...
			Пока Результат.Следующий() Цикл
				ВыборкаНомер = СокрЛП(ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Результат.Номер, Истина,Истина));
			...
			Если Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(СтрокаТаблицы.Заказ)) Тогда
				...
				Если Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(СтрокаТаблицы.Заказ)) 
					...
					НомерДокументаРасчетов 
						= ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(СтрокаТаблицы.ОснованиеПлатежа.Номер, Истина, Истина);
...
Процедура ЗаполнитьРеквизитыДокументаСписаниеБезналичныхДенежныхСредств(ОбъектДокумента, 
	...
	ИначеЕсли ТипЗнч(СтрокаСекции.Контрагент) = Тип("СправочникСсылка.Организации")
	   И ТипЗнч(СтрокаСекции.СчетКонтрагента) = Тип("СправочникСсылка.БанковскиеСчетаОрганизаций") Тогда
		...
		УстановитьСвойство(ОбъектДокумента, 
							"Валюта", 
							СтрокаСекции.СчетКонтрагента.ВалютаДенежныхСредств, , 
							ЭтоНовыйДокумент);
	...
	УстановитьСвойство(ОбъектДокумента, "НомерВходящегоДокумента", 
						ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(СтрокаСекции.НомерДок, Истина, Истина), , ЭтоНовыйДокумент);
