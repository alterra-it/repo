...
Функция ИмяТаблицыНастроек(АТС)
	...
	ИначеЕсли АТС = Перечисления.ДоступныеАТС.Энфорта
		...
		//++Astr
		ИЛИ АТС = Перечисления.ДоступныеАТС._1Servic_Астериск
		//--
...
Процедура ПриВыбореАТС(ВыбраннаяАТС = Неопределено)
	...
	//++Astr
	ИспользоватьАТС_Астериск              = ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС._1Servic_Астериск");
	//--
...
Процедура УправлениеФормой()
	...
	//++Astr
	ИЛИ ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС._1Servic_Астериск");
	
	Если ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС._1Servic_Астериск")  Тогда	
		Элементы.АТС_Itoolabs_АдресОблачнойАТС.Видимость               =Ложь;
		Элементы.АТС_Itoolabs_КлючДляАвторизацииВОблачнойАТС.Видимость =Ложь;
		Элементы._1Servic_Астериск.Видимость                           =Истина;
	Иначе
		Элементы.АТС_Itoolabs_АдресОблачнойАТС.Видимость               =Истина;
		Элементы.АТС_Itoolabs_КлючДляАвторизацииВОблачнойАТС.Видимость =Истина;
		Элементы._1Servic_Астериск.Видимость                           =Ложь;
	КонецЕсли;
	//--
...
&НаКлиенте
Процедура _1Servic_АстерискНажатие(Элемент)
	ОткрытьФорму("Обработка._1Servic_НастройкаАстериск.Форма.Форма",,ЭтотОбъект);
КонецПроцедуры

&НаКлиенте
Процедура АТС_АстерискЛогоНажатие(Элемент)
	//ПерейтиПоСсылке(ПредопределенноеЗначение("Перечисление.ДоступныеАТС._1Servic_Астериск"));
КонецПроцедуры

&НаКлиенте
Процедура АТС_АстерискСсылкаНажатие(Элемент)	
	//ПерейтиПоСсылке(ПредопределенноеЗначение("Перечисление.ДоступныеАТС._1Servic_Астериск"));	
КонецПроцедуры

&НаКлиенте
Процедура ИспользоватьАТС_АстерискПриИзменении()	
	ПриВыбореАТС(ПредопределенноеЗначение("Перечисление.ДоступныеАТС._1Servic_Астериск"));
	ПрочитатьЗначенияНастроек();	
КонецПроцедуры
