...
&НаКлиенте
Процедура ПоказатьОкноВходящегоВызова(НомерС,НомерНа) Экспорт
	
	 Если СтрДлина(НомерС)>4 Тогда
		 
		 СтруктураКлиентов = ПростыеЗвонки_ОбщийМодуль_Сервер.ПолучитьКлиентаПоНомеруТелефона(НомерС);
		 
		 ПараметрыДопФормы = Новый Структура();
		 ПараметрыДопФормы.Вставить("СтруктураКлиентов",СтруктураКлиентов);
		 ПараметрыДопФормы.Вставить("НомерТелефона",НомерС);
		 ПолучитьФорму("ОбщаяФорма.ПростыеЗвонки_ДопФорма",ПараметрыДопФормы,,Новый УникальныйИдентификатор()).Открыть();
...
&НаКлиенте
Процедура _ПодключитьКомпоненту(Команда) 
