...
Процедура СкопироватьВБуфер(Команда)	
	НомерТелефона_ = НомерТелефона;
	АльтерраОбщегоНазначения.ПривестиКФорматуНомерТелефона(НомерТелефона_);
	HTML = Новый COMОбъект("htmlfile");
	HTML.parentWindow.clipboardData.setData("Text", НомерТелефона_);	
КонецПроцедуры
